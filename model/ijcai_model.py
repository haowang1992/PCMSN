...
class Baseline(nn.Module):
	def __init__(self, params_model):
		...
		self.ske_enc = Generator(in_dim=self.feature_size, out_dim=self.dim_out, noise=False, use_dropout=True,
								   use_prelu=True, device=self.device, drop=0.5)
		self.img_enc = Generator(in_dim=self.feature_size, out_dim=self.dim_out, noise=False, use_dropout=True,
								   use_prelu=True, device=self.device, drop=0.5)
		...
		self.img_rec = nn.Sequential(
				nn.Linear(in_features=self.dim_out, out_features=256),
		...
		self.classifier_domain = nn.Linear(self.dim_out, 3, bias=False)
		for param in self.classifier_domain.parameters():
			param.requires_grad = False
		with torch.no_grad():
			self.classifier_domain.weight.div_(torch.norm(self.classifier_domain.weight, dim=1, keepdim=True))
		...
		self.optimizer_gen = optim.Adam(list(self.sem_enc.parameters()) + list(self.ret.parameters()) +
										list(self.ske_enc.parameters()) + list(self.img_enc.parameters()) +
										list(self.ske_rec.parameters()) + list(self.img_rec.parameters()),
										lr=self.lr)
	...
	def forward(self, sketch, image, semantic, label_cls):
		...
		self.ske_domain = self.ske_enc(self.sketch_feature)
		self.img_domain = self.img_enc(self.image_feature)
		self.ske_ret = self.ret(self.sketch_semantic)
		self.img_ret = self.ret(self.image_semantic)

		#self.img_rec_1 = self.img_rec(torch.cat((self.ske_ret, self.img_domain), 1))
		#self.img_rec_2 = self.img_rec(torch.cat((self.img_ret, self.img_domain), 1))
		self.img_rec_1 = self.img_rec(self.ske_ret + self.img_domain)
		self.img_rec_2 = self.img_rec(self.img_ret + self.img_domain)
		self.ske_rec_1 = self.ske_rec(self.ske_ret + self.ske_domain)
		self.ske_rec_2 = self.ske_rec(self.ske_ret + self.ske_domain)
		#self.ske_rec_1 = self.ske_rec(torch.cat((self.ske_ret, self.ske_domain), 1))
		#self.ske_rec_2 = self.ske_rec(torch.cat((self.img_ret, self.ske_domain), 1))

	def backward(self, semantic, label_cls):
		#Cross-reconstruction loss
		loss_img_rec = self.criterion_rec(self.img_rec_1, self.image_feature ) + self.criterion_rec(self.img_rec_2, self.image_feature)
		loss_ske_rec = self.criterion_rec(self.ske_rec_1, self.sketch_feature) + self.criterion_rec(self.ske_rec_2, self.sketch_feature)
		loss_rec = self.lambda_rec*(loss_img_rec + loss_ske_rec)	 
		...
		# Classification loss
		label_ske_domain = torch.zeros(label_cls.size()).cuda().long()
		label_img_domain = torch.ones(label_cls.size()).cuda().long()
		label_sem_domain = torch.ones(label_cls.size()) + 1
		label_sem_domain = label_sem_domain.cuda().long()
		...
		loss_domain_cls = self.criterion_cls(self.classifier_domain(self.img_domain), label_img_domain) + \
							self.criterion_cls(self.classifier_domain(self.ske_domain), label_ske_domain) + \
							self.criterion_cls(self.classifier_domain(self.img_ret), label_sem_domain) + \
							self.criterion_cls(self.classifier_domain(self.ske_ret), label_sem_domain)

		loss_cls = self.lambda_ret_cls * loss_ret_cls + self.lambda_domain_cls * loss_domain_cls


		loss_gen = loss_gen_adv  + loss_rec + loss_cls
		...
		loss = {'gen': loss_gen, 'disc': loss_disc, 'gen_adv': loss_gen_adv, 'ret_cls': loss_ret_cls, 'domain_cls': loss_domain_cls, 'rec': loss_rec}
		return loss
